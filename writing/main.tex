...
\caption{\textbf{Teach-by-Showing pipeline.} Camera frames are encoded by a frozen V-JEPA~2 encoder into 1024-dim latents. During training, an ensemble of 5 dynamics MLPs and a reward model are fit in latent space. At test time, a demonstration is encoded as a goal trajectory $\mathbf{z}_{\text{goal}}[T]$, and a CEM planner selects actions that minimize goal distance while maximizing predicted reward and penalizing ensemble disagreement.}
...
\textbf{Embedding properties.} Across all tasks, V-JEPA embeddings exhibit high temporal coherence (consecutive-frame cosine similarity $>$0.998) while maintaining discriminability (random-pair cosine similarity $\approx$0.95--0.997). Linear probes confirm R$^2 = 0.86$ for object position and R$^2 = 0.89$ for object size/depth (Figure~\ref{fig:probe}).

\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{figures/probe_summary.png}
\caption{\textbf{V-JEPA~2 linear probe results.} A single linear layer trained on frozen V-JEPA features can predict object XY position (R$^2$=0.86), object size (R$^2$=0.89), and object class (88.4\% accuracy) far above random baselines.}
\label{fig:probe}
\end{figure}
...
\FOR{$i = 1$ to $N_{\text{iter}}$}
    ...
    \FOR{each horizon step $h$}
        ...
        \STATE $S^{(m)} \mathrel{+}= \alpha \cdot \hat{r}_h - \|\hat{\mathbf{z}}_{t+h}^{(m)} - \mathbf{z}_{t+h}^{\text{goal}}\|_2 - \beta \cdot \overline{\sigma}_k(\hat{\mathbf{z}}^{(k,m)})$ \hfill \textit{(scoring)}
    \ENDFOR
...
\begin{itemize}
    ...
    \item \textbf{Uncertainty penalty}: Weight $\beta = 2.0$ penalizes ensemble disagreement (mean per-dimension standard deviation across ensemble members), preventing exploitation of dynamics model errors.
\end{itemize}
...
We evaluate on 7 tasks from the DeepMind Control Suite~\citep{tassa2018deepmind}. Figure~\ref{fig:reacher} shows an example environment.

\begin{figure}[h]
\centering
\includegraphics[width=0.25\linewidth]{figures/reacher_frame.png}
\caption{\textbf{Reacher\_easy task.} The 2-joint arm (yellow) must reach the red target. Camera frames at 224$\times$224 are encoded by V-JEPA~2.}
\label{fig:reacher}
\end{figure}
...
Dynamics models are trained for 100 epochs with AdamW ($\text{lr}=3\!\times\!10^{-4}$, weight decay $10^{-5}$), cosine LR schedule, gradient clipping at 1.0, and 90/10 train/val split. Each ensemble member uses bootstrap resampling. Reward models use identical hyperparameters. Figure~\ref{fig:dynamics_loss} shows a representative training curve.

\begin{figure}[h]
\centering
\includegraphics[width=0.6\linewidth]{figures/dynamics_loss.png}
\caption{\textbf{Dynamics model training curve.} MSE loss in V-JEPA latent space converges rapidly, reaching $<$0.001 within 100 epochs.}
\label{fig:dynamics_loss}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{figures/cem_metrics.png}
\caption{\textbf{CEM planning in action (reacher\_easy).} \emph{Left:} Latent distance to goal trajectory decreases over the episode as the planner tracks the demonstration. \emph{Right:} Cumulative environment reward begins accumulating once the agent approaches the target (around step 100).}
\label{fig:cem_metrics}
\end{figure}
...
The cartpole demonstrations score near-zero reward (the heuristic policy largely fails at swingup, averaging $\sim$1.0 cumulative reward). Yet the CEM agent scores 14.4, an emergent capability arising from dynamics-guided planning rather than demonstration imitation. The agent discovers an effective swingup strategy despite never observing one.
