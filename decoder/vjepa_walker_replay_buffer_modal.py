"""
Experiment 10 — Walker-Walk: Robust Replay Buffer
=======================================================================
Root cause of Exp 8 & 9 collapse: The script was looking for an invalid cache file, causing the offline dataset to be empty. The dynamics model then destructively overwrote its global knowledge using only the ~10k on-policy states.
...
# ── Modal setup ─────────────────────────────────────────────────────────────
app = modal.App("vjepa-walker-replay-buffer-dyna")
...
def embed_chunk(start_idx, end_idx):
    import os
    import numpy as np
    import torch
    import PIL.Image
    ...
    model = AutoModel.from_pretrained("facebook/vjepa2-vitl-fpc64-256", cache_dir="/weights/hf", torch_dtype=torch.float16).to(device)
    model.eval()
    
    print(f"[{start_idx}:{end_idx}] Loading frames_np from disk...")
    frames_np = np.load("/rollouts/walker_walk/frames.npy", mmap_mode='r')
...
def walker_reward_dyna_experiment():
    ...
    # ── [2] Load Exp6 offline walker data from raw rollouts ──────────────────
    print("\n[2] Loading offline raw walker frames from /rollouts/walker_walk...")
    cache_file = "/cache/embeddings_cache_walker.pt"
    actions_np = np.load("/rollouts/walker_walk/actions.npy")
    A  = torch.tensor(actions_np[:-1])
    t0 = time.time()
    
    if os.path.exists(cache_file):
        print("    Found existing offline embeddings cache! Loading...")
        cache = torch.load(cache_file, map_location="cpu")
        Z  = cache["z"]
        Zn = cache["z_next"]
        R  = cache["r"]
        print(f"    Loaded {len(Z)} offline transitions.")
    else:
        frames_np  = np.load("/rollouts/walker_walk/frames.npy")
        print(f"    Loaded {len(frames_np)} raw frames in {(time.time()-t0):.1f}s")
        
        print("    Embedding 20,000 offline frames across 10 workers in parallel...")
        t0 = time.time()
        
        chunk_size = 2000
        ranges = [(i, min(i+chunk_size, len(frames_np))) for i in range(0, len(frames_np), chunk_size)]
        
        # Starmap distributes the chunks across up to 10 workers
        ...
        # The offline dataset from Exp 6 did not have rewards computed for our custom task, 
        # so we only use it to regularize the dynamics model.
        Z  = Z_all[:-1]
        Zn = Z_all[1:]
        R  = torch.zeros(len(Z)) # placeholder, not used by reward head training
        
        # Cache to volume
        torch.save({"z": Z, "a": A, "z_next": Zn, "r": R}, cache_file)
        print("    Saved offline embeddings to cache volume.")
    ...
    for rnd in range(1, N_DYNA_ROUNDS + 1):
        ...
        # Re-train reward head fully on all cumulative labelled data
    ...
    fig.suptitle("Exp 10 — Walker-Walk Robust Replay Buffer + Dyna",
    ...
    plt.tight_layout()
    chart_path = "/cache/walker_replay_buffer_results.png"
    plt.savefig(chart_path, dpi=150, bbox_inches="tight"); plt.close()

    # local copy
    chart_local = "/cache/walker_replay_buffer_results.png"

    print(json.dumps(results, indent=2))
    with open("/cache/walker_replay_buffer_results.json", "w") as f:
