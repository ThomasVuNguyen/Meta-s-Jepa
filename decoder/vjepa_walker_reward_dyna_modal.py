...
=======================================================================
Root cause of Exp 7's 50% plateau: CEM used a latent-variance proxy that
doesn't correlate with walker-walk's actual reward (forward velocity × upright).

Fix:
  1. During on-policy rollout collection, record the REAL dm_control reward
     at every step alongside the V-JEPA latent embedding.
  2. Train a lightweight reward head: r_hat = RewardMLP(z_t)
  3. Use r_hat as the CEM objective instead of the proxy.
  4. Run 2 Dyna rounds of warm-start fine-tuning from the Exp6 checkpoint.

Expected outcome: win-rate jumps from 50% → 70%+
"""
...
# ── Modal setup ─────────────────────────────────────────────────────────────
app = modal.App("vjepa-walker-reward-dyna")
...
def walker_reward_dyna_experiment():
    ...
    print("=" * 70)
    print("META-S-JEPA  Experiment 8: Walker-Walk Task-Aligned Reward Head")
    print("=" * 70)
    ...
    results = {
        "experiment": 8,
        "env": "walker-walk",
        "approach": "task-aligned reward head + warm-start dyna",
    ...
    fig.suptitle("Exp 8 — Walker-Walk Task-Aligned Reward Head + Dyna Warm-Start",
    ...
    plt.tight_layout()
    chart_path = "/cache/walker_reward_dyna_results.png"
    plt.savefig(chart_path, dpi=150, bbox_inches="tight"); plt.close()

    # local copy
    chart_local = "/cache/walker_reward_dyna_results.png"

    print(json.dumps(results, indent=2))
    with open("/cache/walker_reward_dyna_results.json", "w") as f:
